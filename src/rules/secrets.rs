...
pub fn secret_rules() -> Vec<SecretRule> {
    ...
    let defs: Vec<(&str, &str, Severity, &str)> = vec![
        ...
        ("github-oauth", "GitHub OAuth Token", Critical,
            r#"gho_[A-Za-z0-9]{36}"#),
        ("github-app-token", "GitHub App Token", High,
            r#"(?:ghs|ghr)_[A-Za-z0-9]{36}"#),
        ("github-fine-grained", "GitHub Fine-Grained PAT", Critical,
            r#"github_pat_[A-Za-z0-9_]{82}"#),

        // GitLab
        ("gitlab-pat", "GitLab Personal Access Token", Critical,
            r#"glpat-[A-Za-z0-9\-]{20,}"#),
        ("gitlab-pipeline", "GitLab Pipeline Token", High,
            r#"glptt-[A-Za-z0-9\-]{20,}"#),
        ("gitlab-runner", "GitLab Runner Token", High,
            r#"glrt-[A-Za-z0-9\-]{20,}"#),
        ...
        // Stripe
        ("stripe-secret", "Stripe Secret Key", Critical,
            r#"sk_live_[0-9a-zA-Z]{24,}"#),
        ("stripe-restricted", "Stripe Restricted Key", High,
            r#"rk_live_[0-9a-zA-Z]{24,}"#),

        // Google
        ("google-api-key", "Google API Key", High,
            r#"AIza[0-9A-Za-z\-_]{35}"#),
        ("google-oauth-id", "Google OAuth Client ID", Medium,
        ...
        // Shopify
        ("shopify-token", "Shopify Access Token", High,
            r#"shpat_[a-fA-F0-9]{32}"#),
        ("shopify-secret", "Shopify Shared Secret", High,
        ...
        ("private-key-dsa", "DSA Private Key", Critical,
            r#"-----BEGIN DSA PRIVATE KEY-----"#),
        ("private-key-ec", "EC Private Key", Critical,
            r#"-----BEGIN EC PRIVATE KEY-----"#),
        ("private-key-openssh", "OpenSSH Private Key", Critical,
            r#"-----BEGIN OPENSSH PRIVATE KEY-----"#),
        ("private-key-pgp", "PGP Private Key Block", Critical,
            r#"-----BEGIN PGP PRIVATE KEY BLOCK-----"#),
        ("private-key-generic", "Generic Private Key", Critical,
            r#"-----BEGIN PRIVATE KEY-----"#),
        ("private-key-encrypted", "Encrypted Private Key", High,
            r#"-----BEGIN ENCRYPTED PRIVATE KEY-----"#),

        // Database URIs
        ("db-uri-password", "Database URI with Password", Critical,
            r#"(?i)(?:postgres|mysql|mongodb|redis|amqp|mssql)://[^:]+:[^@\s]{3,}@[^\s]+"#),

        // Generic patterns
        ("basic-auth-url", "Basic Auth in URL", High,
            r#"https?://[^:]+:[^@\s]{3,}@[^\s]+"#),
        ("bearer-token", "Hardcoded Bearer Token", High,
            r#"(?i)(?:authorization|bearer)\s*[:=]\s*['"]?bearer\s+[A-Za-z0-9\-_.~+/]{20,}['"]?"#),

        // Password assignments
        ("password-assignment", "Hardcoded Password", High,
            r#"(?i)(?:password|passwd|pwd|secret|token|api_key|apikey|api_secret|access_token|auth_token)\s*[:=]\s*['"][^'"]{8,}['"]"#),

        // JWT
        ("jwt-token", "Hardcoded JWT", Medium,
        ...
        // HashiCorp
        ("vault-token", "HashiCorp Vault Token", Critical,
            r#"hvs\.[A-Za-z0-9]{24,}"#),
        ("terraform-token", "Terraform Cloud Token", High,
        ...
        // Sentry
        ("sentry-dsn", "Sentry DSN", Medium,
            r#"https://[0-9a-f]{32}@[a-z0-9.]+\.ingest\.sentry\.io/[0-9]+"#),

        // Age encryption
        ("age-secret-key", "Age Secret Key", Critical,
            r#"AGE-SECRET-KEY-1[A-Z0-9]{58}"#),

        // Doppler
        ("doppler-token", "Doppler Token", High,
