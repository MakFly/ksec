#!/bin/sh
# Pose (ou retire) le timer systemd --user qui tient ksec à jour sur ce poste.
#
...
#
# systemd --user et non root : ksec s'installe dans ~/.local/bin, il n'y a
...
command -v systemctl >/dev/null 2>&1 ||
  ...
  Sur macOS, l'équivalent est un launchd agent ; sur les autres, un cron :
    0 4 * * *  KSEC_ONLY_IF_NEWER=1 sh $REPO/install.sh"

if [ "${1:-}" = "--uninstall" ]; then
	systemctl --user disable --now ksec-update.timer 2>/dev/null || true
	rm -f "$UNIT_DIR/ksec-update.timer" "$UNIT_DIR/ksec-update.service"
	systemctl --user daemon-reload
	say "✓ timer retiré (ksec lui-même reste installé)"
	exit 0
fi
...
sed "s#@INSTALL_SH@#$REPO/install.sh#" \
	"$HERE/ksec-update.service" >"$UNIT_DIR/ksec-update.service"
cp "$HERE/ksec-update.timer" "$UNIT_DIR/ksec-update.timer"

systemctl --user daemon-reload
systemctl --user enable --now ksec-update.timer

say ""
say "✓ ksec se met à jour tout seul — une vérification par jour."
say ""
say "  état      systemctl --user list-timers ksec-update.timer"
say "  forcer    systemctl --user start ksec-update.service"
say "  journal   journalctl --user -u ksec-update.service -n 20"
